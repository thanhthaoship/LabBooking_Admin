...
import {
  deleteRoomMaintainSchedule,
  getRoomMaintainSchedules,
...
import RoomMaintainScheduleDialogForm from "./components/RoomMaintainScheduleDialogForm";
...
export default function RoomMaintainSchedulesPage() {
  ...
  const query: GetAllRoomMaintainSchedulesQuery = useMemo(
    () => ({
      searchPhrase: searchPhrase || undefined,
      status: status === "All" ? undefined : (status as RoomMaintainStatus),
      pageNumber: page + 1,
      pageSize,
      sortBy,
      sortDirection,
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getRoomMaintainSchedules(query)
      .then((res) => {
        if (!active) return;
      ...
      .catch((e) => {
        if (!active) return;
        setError(e?.message ?? "Không thể tải danh sách lịch bảo trì phòng");
      })
      .finally(() => {
        if (!active) return;
        setLoading(false);
  ...
  const filtered = useMemo(() => {
    const list = (data?.items ?? []).filter((x) => {
      const label = `${x.labRoomId ?? ""} ${x.description ?? ""}`.toLowerCase();
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteRoomMaintainSchedule(confirmDelete.id);
      setConfirmDelete({ open: false, id: null });
      setRefreshSeq((s) => s + 1);
    } catch (e: any) {
      setError(e?.message ?? "Xóa lịch bảo trì phòng thất bại");
      setConfirmDelete({ open: false, id: null });
    }
  };
  ...
  return (
    <Box sx={{ px: { xs: 2, md: 4 }, py: 3, bgcolor: "background.default" }}>
          ...
          {filtered.length === 0 ? (
            <Box sx={{ py: 6 }}>
              <EmptyState
                ...
                description="Hãy thêm lịch bảo trì phòng để bắt đầu"
      ...
      <RoomMaintainScheduleDialogForm
        open={createOpen}
